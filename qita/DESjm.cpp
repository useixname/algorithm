...
void des_encrypt_block(const int *in64, int *out64, int subkeys[16][48]) {
    int ipout[64], L[32], R[32];
    permute(IP,64,in64,64,ipout);
    memcpy(L,ipout,32*sizeof(int));
    memcpy(R,ipout+32,32*sizeof(int));
...
// DES 解密块：反转子密钥顺序
void des_decrypt_block(const int *in64, int *out64, int subkeys[16][48]) {
    int rev[16][48];
    for(int i=0;i<16;i++)
        memcpy(rev[i], subkeys[15-i], 48*sizeof(int));
    des_encrypt_block(in64, out64, rev);
}

// 辅助：hex 字符串 -> 64-bit 数组
...
// 辅助：64-bit 数组 -> hex 字符串
void bits_to_hexstr(const int *bits64, char *hex) {
    for(int i=0;i<16;i++){
...
int main(){
    char ctHex[17], keyHex[17], ptHex[17];
    int ciphertext[64], key64[64], subkeys[16][48], plaintext[64];
    ...
    hexstr_to_bits(ctHex, ciphertext);
    hexstr_to_bits(keyHex, key64);
    generate_subkeys(key64, subkeys);

    des_decrypt_block(ciphertext, plaintext, subkeys);
    bits_to_hexstr(plaintext, ptHex);
